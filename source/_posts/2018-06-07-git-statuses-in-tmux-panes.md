...
One downside of this approach may be increased power usage and CPU overhead of running these git commands every second, for each pane. But even with a bunch of panes open, it didn't noticeably affect power or CPU consumption on my laptop. Also, you could of course modify the script below to show other information you might want on a per-pane basis: active nodeJS version, ssh details, etc.
